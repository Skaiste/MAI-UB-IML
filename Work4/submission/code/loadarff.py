# Load arff file taken from https://raw.githubusercontent.com/scipy/scipy/refs/tags/v1.14.1/scipy/io/arff/_arffread.py
# Edited some parts marked with '### EDIT'
...
def _loadarff(ofile):
    ...
    def generator(row_iter, delim=','):
        ...
        dialect = None
        for raw in row_iter:
            ### EDIT (by Skaiste): some rows contain invalid characters -> removing them
            raw = raw.replace('\x00', '') # zero character
            ...
            ### EDIT (by Skaiste): The data has some errors with half of different rows
            ###                    joined together and incomplete rows

            if len(row) > len(attr):
                # Assuming the row is larger, since some of the data belongs to a new entry
                # => parse until the data looks like it doesn't belong to the row
                parsed_data = []
                cutoff = -1
                for i in elems:
                    try:
                        pd = attr[i].parse_data(row[i])
                ...
                # attempt to load the other entry from the row
                if cutoff != -1:
                    new_entry = []
                    for i in elems:
                        try:
                            pd = attr[i].parse_data(row[cutoff])
                            new_entry.append(pd)
                            cutoff += 1
                        except ValueError as err:
                            if attr[i].type_name == "nominal":
                    ...
                    if set(new_entry).issubset({"?", np.nan}):
                        yield tuple(new_entry)
            elif len(row) < len(attr):
                # Assuming row is incomplete - collecting as much of the row as possible
                # and filling the rest as '?' or np.nan
                parsed_data = []
                ridx = 0
                for i in elems:
                    try:
                        pd = attr[i].parse_data(row[ridx])
                        parsed_data.append(pd)
                        ridx += 1
